...
export type ItemControl = typeof ITEM_CONTROLS[keyof typeof ITEM_CONTROLS];
...
interface ItemStatusDict {
  name: string;
  description: string;
}

export const CONTROLS_TEXTS_DICT: Record<ItemControl, ControlText> = {
  [ITEM_CONTROLS.TITLE]: {
    linkName: 'Title',
  ...
  [ITEM_CONTROLS.STATUS]: {
    ...
    text: "An item's status is either published, unlisted, or unpublished. <ul class='list-decimal list-inside'>" +
      `${Object.entries(ITEM_STATUSES_DICT as Record<number, ItemStatusDict>).map(([status, info]) => (
        `<li>${info.name}: ${info.description}</li>`)).join('')}` +
      "</ul>",
  },
};